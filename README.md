...
5. **题库**:所有账号共用同一个题库(存于服务端 `bank` 表),每次答题后自动收录新题,越用越全;仓库自带初始题库 `bank.json`(108 题),首次启动(题库为空)自动导入。管理员可在「管理 → 题库」查看统计、导出 JSON、导入题库(兼容油猴脚本导出的 `{"题干": ["答案"]}` 格式)。题库只存在本机 `app.db`,不上传任何云端,本地实例之间互不影响。
